...
export function createTypeRecoveryReport<T>(candidates:readonly TypeRecoveryCandidate<T>[]):TypeRecoveryReport<T> { const validation=validateGlobalTypeRecovery(candidates);const strongest=candidates.filter(c=>c.authoritative).sort((a,b)=>((b as any).confidence??0)-((a as any).confidence??0));return {validation,candidateCount:candidates.length,resolvedValue:validation.authoritative?strongest[0]?.value:undefined}; }
